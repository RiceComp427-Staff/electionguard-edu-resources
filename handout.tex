...
If you're using a tool like \python{PyCharm}\kotlin{IntelliJ}, it will
{\em try} to do all of this
...
\python{PyCharm might offer to set up a Python ``virtual environment''
  for you, but you should instead let {\tt make} do it for you and then tell
...
We can improve efficiency without significantly impacting security by
...
How do you select $p$? One might then choose $p$ to be the largest 4096-bit prime such that
...
which involves editing code in \pythontt{elgamal.py}\kotlintt{ElGamal.kt}.
Everything else you'll do is in
\pythontt{simple\_elections.py}\kotlintt{SimpleElections.kt}
...
requirements. You then say \fbox{\tt git tag part1} and \fbox{\tt git push
...
\pythontt{Pipfile}\kotlintt{build.gradle}.
...
Right now, the code as you have it is using the modular
exponentiation that's built into \python{Python / GnuMP}\kotlin{the
  JVM's BigInteger class}, and those modular exponentiation operations
...
\begin{itemize}
\item To optimize \pythontt{g\_pow\_p}\kotlintt{gPowP}, We might
...
\item Instead, we're going to do even better by doing additional
  ...
  Even better: you shouldn't store each entry where all eight bits
  are zero, because $g^0 = 1$. Instead, just add a special case
  before the lookup, giving you a $1/256$ chance that you
  don't need to do each of those multiplies.
...
\item The more effort and storage we dedicate to precomputation, the
  ...
  would shrink the computation from 31 multiplies down to 25
  multiplies. The real ElectionGuard will (likely) end using different
  table sizes for different applications. On an embedded computer with limited
  memory, an 8-bit lookup table still yields a significant speedup,
  which on a server-class machine processing millions of ballot
  ciphertexts, any modest speedup we can get might be worth spending gigabytes
  of memory on the precomputation.

  For {\em you}, for this project, just do 8-bit lookups. This is
  much easier to implement because it's easy to convert an element to
  an array of bytes.
...
\item You can repeat exactly the same process for using $g^a$ (the public
  ...
  structures? How should you reengineer the existing \pythontt{pow\_p}\kotlintt{powP}\ function?
...
\item \python{As you work on {\tt g\_pow\_p} and {\tt pow\_p} in {\tt
      group.py}, you'll most likely want to take advantage of the {\tt
      q\_to\_bytes} function, already present, to
      extract the bytes that you'll be using.}
      \kotlin{As you work on {\tt gPowP} and {\tt powP} in {\tt 
      Group.kt}, you'll most likely want to take advantage of
      BigInteger's {\tt toByteArray}
      method\footnote{\url{https://docs.oracle.com/javase/8/docs/api/java/math/BigInteger.html\#toByteArray--}},
      to extract the bytes that you'll be using.}
      Note that you won't always get 32 bytes out. Smaller numbers
      will yield shorter arrays. You'll find it helpful to write
      unit tests to exercise these cases.
