...
Normal ``exponential'' ElGamal encryption, invented in 1985 by Tahir
...
The real ElectionGuard supports a sophisticated ``threshold
cryptography'' system
...
you could theoretically use a disjunctive proof again, but we 
want to support $k$-of-$n$ candidates being selected. That would make a disjunctive proof
...
of the various data structures in
...
of $m=1$ is actually one, based on our knowledge of the encryption
...
encryption, after we multiply it with $g^{-1}$ (i.e., canceling
out the $g^m$ where $m=1$).
